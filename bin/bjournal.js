...
`
  ...
  .option(
    "-d, --date <string>",
    "Specify a custom date (e.g., 2025-08-01)",
    isoLocalDate()
  )
...
program
  ...
  .alias("l")
  .description(
    "View todos for a specific day or a recent range (default: today)"
  )
  .usage(
    `
Examples:
...
program
  ...
  .arguments("<index...>")
  .description(
    "Mark one or more todos as important (bold); marking multiple times increases emphasis"
  )
  .usage(
    `
Examples:
...
`
  ...
  .option(
    "-d, --date <string>",
    "Specify the date the todo currently exists on"
  )
  ...
  .action((indexes, option) => {
    ...
    const isoDate = toIsoLocalDate(dateOption ? dateOption : date);
    const isoForwardDate = toIsoDate(forwardDate);
...
program
  ...
  .description(
    "Manage default todos that are automatically added when inserting or listing a day"
  )
  .usage(
    `
Examples:
...
`
  ...
  .option(
    "--get-backup <path>",
    "Save a backup of your data to a specified path"
  )
  .action((option) => {
    if (option.showPath) {
      getDataPath();
